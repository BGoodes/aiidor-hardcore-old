...
public class Short_Bow extends AiidorEnchant  implements Listener {
	
	public Short_Bow(NamespacedKey id) {
		super(id);
	}
	...
	public Double getBonusDamage(Entity target, Entity shooter, int level)  {
		...
		if ((4 * level) - distance * 2 <= 0) return 0.0; 
		return (4 * level) - distance * 2;
	}
	...
	public boolean conflictsWith(Enchantment enchant) {
		if (enchant.equals(AiidorEnchant.LONG_BOW)) return true;
		return false;
	}
