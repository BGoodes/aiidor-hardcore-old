...
public class Spider_Events implements Listener {
	...
	public Spider_Events(Main main) {
		this.main = main;
		...
		if (e.getDamager().getType() == EntityType.SPIDER) {
			...
			if (new Random().nextInt(3) == 0) {
				...
				if (web.getType() == Material.AIR) {
					...
					RemoveWeb webTask = new RemoveWeb(main, web);
					webTask.runTaskTimer(main, 0, 20);
					
					main.webTasks.add(webTask);
				}
	...
	public void PlayerBreakWebEvent(BlockBreakEvent e) {
		...
		if (!main.webTasks.isEmpty()) {
			
			Block b = e.getBlock();
			
			for (RemoveWeb webTask : main.webTasks) {
